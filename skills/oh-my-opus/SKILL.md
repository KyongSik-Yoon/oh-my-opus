...
Three gates all have to pass, so it is quiet by default: the session model is **Opus 5 or Fable 5**, the flag does not say `overlay=off`, and **the project opted in** by creating a marker file `.claude/oh-my-opus` in the repo (any parent directory of the session cwd counts). Without that marker nothing is injected — which is what keeps the plugin from silently rewriting the rules of a repository that never asked.
