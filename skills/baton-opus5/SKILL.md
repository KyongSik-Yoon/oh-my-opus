---
name: baton-opus5
argument-hint: "[on [maxagents=auto|0-99] [recap=on|off] | off | maxagents auto|0-99 | recap on|off | status]"
...
- `recap on|off` — rewrite only that line, keep the current `maxagents`.
...
Missing flag file means the plugin is inert: no cap, no recap, nothing written. The `recap` hook is another stage and is not wired in yet; setting `recap` still records your preference.
